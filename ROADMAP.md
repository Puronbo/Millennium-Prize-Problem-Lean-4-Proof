...
- `UniversalSingularity/BSD37a1.lean` — **new**: fully proved concrete
  ...
  `−2P = U`, `−3P = S`, `−S = T`), the group-theoretic no-torsion form
  `(n : ℕ) • p ≠ 0` for `n = 2, ..., 9`, and no torsion of order `2` through
  `9` for the generator `P`. **Zero `sorry`s** — the first fully proved module
...
1. **DONE:** `UniversalSingularity/BSD37a1.lean` fully proves the concrete
   ...
   `2T = X` and the independent check `3T = 9P`), the inverse pairs (`P + R = 0`,
   `Q + U = 0`, `T + S = 0`, plus `−2P = U`, `−3P = S`, `−S = T`), the
   group-theoretic no-torsion form `(n : ℕ) • p ≠ 0` for `n = 2, ..., 9`, and
   the absence of torsion of order `2` through `9` for the generator `P`. Zero
   `sorry`s.
